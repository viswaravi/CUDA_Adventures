...
void pageableMemoryAdditionLarge(unsigned long long array_len,
  ...
  size_t total_mem = free_mem * free_mem_threshold; // Limit memory usage in GPU
  ...
  for (int chunk_index = 0; chunk_index < chunk_num; chunk_index++) {
    ...
    addWithCudaMemSafe(h_C + chunk_offset, h_A + chunk_offset,
                       h_B + chunk_offset, chunk_len, use_vectorized_kernel);
  ...
  if (final_chunk_len > 0) {
    ...
    addWithCudaMemSafe(h_C + chunk_offset, h_A + chunk_offset,
                       h_B + chunk_offset, final_chunk_len, use_vectorized_kernel);
  }
...
void streamedVectorAdditionLarge(unsigned long long array_len,
                                 float free_mem_threshold = 0.2,
                                 int num_streams = 4, bool use_vectorized_kernel = false) {
  ...
  unsigned long long chunk_len =
      chunk_max_len - (chunk_max_len % MAX_BLOCK_DIM);
  ...
  for (unsigned long long chunk_start = 0; chunk_start < array_len;
    ...
    for (int stream_index = 0; stream_index < num_streams; stream_index++) {
      unsigned long long chunk_offset =
          chunk_start + (stream_index * chunk_len);
  ...
  const VariantRegistry variants = {
      ...
      },
      {
          ...
          [](const RunConfig &c) {
            ...
            bool use_vectorized = get_bool(c, "--vectorized", false);
            printMemoryRequirements(n);
            pageableMemoryAdditionLarge(
                n, get_float(c, "--free-mem-threshold", 0.2f), use_vectorized);
          },
      },
      {
          "pinned-large",
          ...
          [](const RunConfig &c) {
            ...
            streamedVectorAdditionLarge(
                n, get_float(c, "--free-mem-threshold", 0.2f),
