...
__global__ void reduce4(float *A, float *blockSums, const unsigned long long length) {
    ...
    int tid = threadIdx.x;
    float sum = idx < length ? A[idx] : 0.0f;
    if (idx + blockDim.x < length) {
        sum += A[idx + blockDim.x];
    }
    partialSum[tid] = sum;
...
// Full-array reduction using warp shuffle primitives within each block.
__global__ void warpPrimitives(float *A, float *result, const unsigned long long length) {
    ...
    unsigned long long idx = (blockIdx.x * blockDim.x) + threadIdx.x;
    unsigned long long gridSize = static_cast<unsigned long long>(blockDim.x) * gridDim.x;
    extern __shared__ float partialSum[];
    float sum = 0.0f;
