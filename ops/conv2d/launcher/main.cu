...
{
  const std::vector<ArgSpec> base_args = {{"--image", "path", "res/cuda.png", "Input image path"}};
  const std::vector<ArgSpec> conv_args = {
      {"--image", "path", "res/cuda.png", "Input image path"},
      {"--kernel", "int", "1", "Kernel type: 1=Gaussian, 2=SobelX, 3=SobelY, 4=Sharpen, 5=LoG"},
  };

  const VariantRegistry variants = {
      {"rgb2gray",          "RGB to grayscale conversion",                       base_args, [](const RunConfig &c) { run_convolution(RGB2GREY, c); }},
      {"conv-naive",        "Naive global-memory 2D convolution",                conv_args, [](const RunConfig &c) { run_convolution(TWO_D_CONV_NAIVE, c); }},
      {"conv-shared-single","Single-thread tile loading shared-memory convolution",conv_args,[](const RunConfig &c) { run_convolution(TWO_D_CONV_SHARED_SINGLE, c); }},
      {"conv-shared-coop",  "Cooperative tile loading shared-memory convolution",conv_args, [](const RunConfig &c) { run_convolution(TWO_D_CONV_SHARED_COOP, c); }},
      {"conv-shared-struct","Structured halo-loading shared-memory convolution", conv_args, [](const RunConfig &c) { run_convolution(TWO_D_CONV_SHARED_STRUCT, c); }},
  };
