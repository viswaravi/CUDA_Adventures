...
int main(int argc, char **argv)
{
  ...
  const VariantRegistry variants = {
      {"rotation-naive", "Image rotation with direct bilinear interpolation", rotation_args,
       [](const RunConfig &c) { run_rotation(IMG_ROT_NAIVE, c); }},
      {"texture-interpolation", "Texture interpolation sampling test", texture_args,
       [](const RunConfig &c) { run_rotation(TEX_TEST, c); }},
      {"rotation-bbox-coop", "Rotation with cooperative shared-memory BBox loading", rotation_args,
       [](const RunConfig &c) { run_rotation(IMG_ROT_BBOX_COOP, c); }},
      {"rotation-bbox-strided", "Rotation with strided shared-memory BBox loading", rotation_args,
       [](const RunConfig &c) { run_rotation(IMG_ROT_BBOX_STRIDED, c); }},
  };
